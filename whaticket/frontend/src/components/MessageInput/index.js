...
import { FormControlLabel, Switch, Grid, Typography, List, ListItem, ListItemText, ListItemAvatar, Avatar } from "@material-ui/core";
...
const MessageInput = ({ ticketStatus }) => {
  ...
  const handleUploadMedia = async e => {
    ...
    xhr.open("POST", `/messages/${ticketId}`, true);
    ...
    xhr.onload = () => {
      if (xhr.status === 200) {
        ...
        setLoading(false);
        setUploadProgress(0);
        toastError(xhr.responseText || i18n.t("uploads.toasts.error"));
      }
    };
