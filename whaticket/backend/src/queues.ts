...
function getCampaignValidConfirmationMessages(campaign) {
  const messages = [];
  if (!isEmpty(campaign.confirmationMessage1) && !isNil(campaign.confirmationMessage1)) {
    messages.push(campaign.confirmationMessage1);
  }
  if (!isEmpty(campaign.confirmationMessage2) && !isNil(campaign.confirmationMessage2)) {
    messages.push(campaign.confirmationMessage2);
  }
  if (!isEmpty(campaign.confirmationMessage3) && !isNil(campaign.confirmationMessage3)) {
    messages.push(campaign.confirmationMessage3);
  }
  if (!isEmpty(campaign.confirmationMessage4) && !isNil(campaign.confirmationMessage4)) {
    messages.push(campaign.confirmationMessage4);
  }
  if (!isEmpty(campaign.confirmationMessage5) && !isNil(campaign.confirmationMessage5)) {
...
// Função auxiliar para cálculo dinâmico de delay com componente aleatório
function calculateDynamicDelay(
  index: number,
  baseDelay: moment.Moment,
  longerIntervalAfter: number,
  greaterInterval: number,
  messageInterval: number
): number {
  // Adiciona um atraso aleatório de até 3 segundos (3000 ms)
  const randomExtraDelay = Math.floor(Math.random() * 3000);
  const interval = index > longerIntervalAfter ? greaterInterval : messageInterval;
  baseDelay.add(interval, 'seconds');
  // Calcula a diferença em ms entre o novo baseDelay e agora e soma o atraso aleatório
  return baseDelay.diff(moment()) + randomExtraDelay;
}
...
async function handleProcessCampaign(job) {
  try {
    ...
    if (campaign) {
      const { contacts } = campaign.contactList;
      if (isArray(contacts)) {
        // Mapeia os contatos e configura os dados para cada envio
        ...
        let baseDelay = moment(campaign.scheduledAt); // inicia a partir da data agendada

        const queuePromises = [];
        for (let i = 0; i < contactData.length; i++) {
          // Calcula um delay dinâmico para cada contato
          const delay = calculateDynamicDelay(i, baseDelay, longerIntervalAfter, greaterInterval, messageInterval);
          ...
          logger.info(`Registro enviado pra fila de disparo: Campanha=${campaign.id}; Contato=${contacts[i].name}; delay=${delay}ms`);
        }
    ...
    companies.map(async c => {
      ...
      if (dias < 20) {
        ...
        if (invoice[0]['mycount'] > 0) {
          // Já existe fatura para essa data
        } else {
          const sql = `INSERT INTO "Invoices" (detail, status, value, "updatedAt", "createdAt", "dueDate", "companyId")
                       VALUES ('${plan.name}', 'open', '${plan.value}', '${timestamp}', '${timestamp}', '${date}', ${c.id});`
          await sequelize.query(sql, { type: QueryTypes.INSERT });
          // Aqui também é possível disparar um e-mail ou notificação, se necessário.
        }
      }
    });
  });
...
export async function startQueueProcess() {
  ...
  campaignQueue.add(
    "VerifyCampaigns",
    {},
    { repeat: { cron: "*/20 * * * * *", key: "verify-campaing" }, removeOnComplete: true }
  );
  userMonitor.add(
    "VerifyLoginStatus",
    {},
