...
export const getBodyMessage = (msg: proto.IWebMessageInfo): string | null => {
  try {
    ...
    const types = {
      ...
      // Corrigindo a exibição do nome do documento para evitar duplicação
      documentMessage: (() => {
        const fileName = msg.message?.documentMessage?.fileName;
        const docTitle = msg.message?.documentMessage?.title || msg.message?.documentMessage?.caption;
        if (fileName && docTitle) {
          return fileName === docTitle
            ? `*${fileName}*`
            : `*${fileName}*\n${docTitle}`;
        }
        return fileName || docTitle;
...
const verifyQueue = async (
  ...
  */
  const botText = async () => {
    ...
    let invalidOption = "Desculpe, mas a opção que você escolheu não é válida. Aqui estão suas opções novamente:\n\n" + options;

    // Variantes da mensagem de ajuda
    const helpMessages = [
      "Caso precise, basta digitar o número da opção que deseja escolher!",
      "Fique à vontade para digitar o número da opção que melhor atende às suas necessidades!",
      "Se precisar de ajuda, é só digitar o número da opção que você gostaria de escolher!",
      "Estou aqui para ajudar! Digite o número da opção que você prefere."
    ];

    const additionalHelpMessage = helpMessages[Math.floor(Math.random() * helpMessages.length)]; // Escolhe uma mensagem aleatória

    console.log('textMessage2', textMessage);
    console.log("lastMsg::::::::::::':", contact.number);

    // Verifica se a mensagem atual é válida
    if (!lastMsg?.msg || getBodyMessage(msg).includes('#') || textMessage.text === 'concluido' || (lastMsg.msg !== textMessage.text && !lastMsg.invalid_option)) {
      ...
      lastMsg.invalid_option = false; // Reseta o status de opção inválida
      lastMsg.attempts = 0; // Reseta o contador de tentativas
      map_msg.set(contact.number, lastMsg);
      await verifyMessage(sendMsg, ticket, ticket.contact);

    } else if (lastMsg.invalid_option) {
      // Incrementa o contador de tentativas
      lastMsg.attempts = (lastMsg.attempts || 0) + 1;

      // Verifica se já excedeu o número de tentativas
      if (lastMsg.attempts < 3) {
        // Mensagem de opção inválida
        const sendMsg = await wbot.sendMessage(
          `${contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`,
          { text: invalidOption }
        );
        lastMsg.msg = invalidOption; // Atualiza a última mensagem
        map_msg.set(contact.number, lastMsg);
        await verifyMessage(sendMsg, ticket, ticket.contact);

        // Adiciona um pequeno atraso antes de enviar a mensagem de ajuda
        await new Promise(resolve => setTimeout(resolve, 1000)); // Espera 1 segundo

        // Mensagem adicional de ajuda
        await wbot.sendMessage(
          `${contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`,
          { text: additionalHelpMessage }
        );
      } else {
        // Se já tentou 3 vezes, avisa que não entendeu e reseta
        const finalInvalidMessage = "Parece que não estou conseguindo entender sua escolha. Vamos tentar novamente!\n\n" + options;
        await wbot.sendMessage(
          `${contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`,
          { text: finalInvalidMessage }
        );

        // Reseta o contador e volta ao início
        lastMsg.attempts = 0; // Reseta o contador
        lastMsg.invalid_option = false; // Reseta o status de opção inválida
        lastMsg.msg = ""; // Reseta a última mensagem
        map_msg.set(contact.number, lastMsg); // Atualiza o mapa
      }
    } else {
      // Mensagem de opção inválida inicial
      if (lastMsg.msg !== invalidOption && !lastMsg.invalid_option) {
        lastMsg.invalid_option = true; // Marca que uma opção inválida foi escolhida
        const sendMsg = await wbot.sendMessage(
          `${contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`,
          { text: invalidOption }
        );
    ...
    } // Chave de fechamento para o else

  }; // Chave de fechamento para o botText
