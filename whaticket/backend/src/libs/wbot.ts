      ...
      (async () => {
        ...
        wsocket.ev.on(
          ...
          async ({ connection, lastDisconnect, qr }) => {
            ...
            if (connection === "open") {
              const userId = (wsocket as WASocket).user.id; // Obtendo o ID do usuário
              const number = jidNormalizedUser(userId).split("@")[0]; // Extraindo o número

              await whatsapp.update({
                status: "CONNECTED",
                qrcode: "",
                retries: 0,
                number: number // Atualizando o número no banco de dados
              });
            ...
            if (qr !== undefined) {
              if (retriesQrCodeMap.get(id) && retriesQrCodeMap.get(id) >= 3) {
                ...
                await whatsapp.update({
                  ...
                  status: "qrcode",
                  retries: 0,
                  number: "" // Deixando o número vazio enquanto aguarda a conexão
                });
                const sessionIndex = sessions.findIndex(s => s.id === whatsapp.id);
